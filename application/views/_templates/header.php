            ...
            <small>
                <?php if ($_SESSION['phpback_isadmin'] >= 1) : ?>
                    <a href="<?= base_url() . 'admin/'; ?>">
                        <button type="button" class="btn btn-info btn-xs" style="margin-left:10px;">ADMIN</button>
                    </a>
                <?php endif; ?>

                <span class="logged-as-label"><?= $lang['label_logged_as']; ?></span>
                <span style='color:#999;margin-left:5px;'>
                    <a href="<?= base_url() . 'home/profile/' . $_SESSION['phpback_userid'] . '/' . Display::slugify($_SESSION['phpback_username']); ?>">
                        <?= $_SESSION['phpback_username']; ?>
                    </a>
                </span>
                <a href="<?= base_url() . 'action/logout'; ?>">
                    <button type="button" class="btn btn-danger btn-xs" style="margin-left:10px;"><?= $lang['label_log_out']; ?></button>
                </a>
            </small>
        </div>
    <?php else : ?>
        <div class="pull-right" style="padding-top:12px;padding-right:40px;">
            <a href="<?= base_url() . 'home/login'; ?>">
                <button type="button" class="btn btn-success btn-sm btn-block" style="width:250px"><?= $lang['label_log_in']; ?></button>
            </a>
        </div>
    <?php endif; ?>
