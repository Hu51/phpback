<div class="row">
    <div class="col-md-10 col-md-offset-1 dashboard-center">
        <?php $this->load->view("admin/dashboard/nav"); ?><!-- /navbar -->
        <div>
            <h5>Ideas and comments</h5>
            <ul class="nav nav-tabs">
                <li id="table1" class="active"><a onclick="showtable('newideastable','table1');">New Ideas <span class="badge"><?= $newideas_num; ?></span></a></li>
            ...
            <div id="listing">
                <table id="newideastable" class="table table-condensed" style="">
                    ...
                    <?php foreach ($newideas as $idea): ?>
                        ...
                        ?>">
                            <td>
                                <a href="<?= base_url() . 'home/idea/' . $idea->id . "/" . $freename; ?>" target="_blank"><?= $idea->title; ?></a>
                            </td>
                            <td>
                                    ...
                                    <td style="padding-left:10px;width:250px;vertical-align:top">
                                        <div class="form-group">
                                            <?php foreach ($categories as $category): ?>
                                                <label class="checkbox">
                                                    <input type="checkbox" <?php $s = 'category-' . $category->id;
                                                    if ($form[$s]): ?>checked="checked" <?php endif; ?> name="category-<?= $category->id; ?>" data-toggle="checkbox">
                                                    <?= $category->name; ?>
                                                </label>
                                            <?php endforeach; ?>
                                        </div>
                    ...
                    <?php foreach ($flags as $comment) : ?>
                        <tr>
                            <td>
                                Comment: #<?= $comment['id']; ?>
                                <br>User:
                                <a href="<?= base_url() . 'admin/users/' . $comment['userid']; ?>">#<?= $comment['userid']; ?></a>
                                <br>Idea:
                                <a href="<?= base_url() . 'home/idea/' . $comment['ideaid']; ?>" target="_blank">#<?= $comment['userid']; ?></a>
                            </td>
                            <td>
                                <samp>
                                    <?= $comment['content']; ?>
                            ...
                            <td>
                    <span style="font-size:17px;">Flagged <span class="badge"><?= $comment['votes']; ?></span><span style="font-size:17px;"> times</span>
                    <div class="pull-right">
                      <button name="Delete votes" type="submit" class="btn btn-warning btn-sm"
                              style="width:130px" <?php $temp = base_url() . 'adminaction/deletecomment/' . $comment['id']; ?> onclick="popup_sure('Are you sure you want to delete this comment?','<?= $temp; ?>');">Delete Comment</button>
                      <?php if ($_SESSION['phpback_isadmin'] > 1): ?><a href="<?= base_url() . 'admin/users/' . $comment['userid']; ?>">
                              <button type="submit" class="btn btn-danger btn-sm" style="width:130px">Ban User</button></a><?php endif; ?>
                    </div>
                            </td>
                        </tr>
                    <?php endforeach; ?>
